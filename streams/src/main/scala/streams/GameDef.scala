...
trait GameDef {
  ...
  /**
   * This function returns the block at the end position of
   * the game.
   */
  ...
  case class Block(b1: Pos, b2: Pos) {
    ...
    def legalNeighbors: List[(Block, Move)] = {
      ...
      if(up.isLegal) buf += ((up, Up))
      if(right.isLegal) buf += ((right, Right))
      if(down.isLegal) buf += ((down, Down))
      if(left.isLegal) buf += ((left, Left))
     ...
     */
    //When both poses are on the same game level then the block is standing.
